import json
import math
import random
import threading
import time
import datetime
# pip3 install requests
import requests
import sys
import NoticePush
# 可以改的参数
Quanargs=[
#10-2 7/10/16/20
'key=99E9BFB397BB9E081C3D211644F4D63B80C6629BF10290DBE13C6C11E1DF025FE3774FB37875ED1B720ABB1A6023D3B4_bingo,roleId=_bingo,strengthenKey=4C8818C1732D1A7B9733D611F28BD8B57923ADE220C9B60370F65B533E02E0F8F7A8DB4211A7817EF606DDAD4FA3E67F_bingo',
#19-5 0/10/18/22
'key=E72F2D6FD3B257AE6EAEEF81FEF44D9C3EFB9B5E0F0E11C4562D68BDA7BA69BF5C4E716FB9BBD7B1678E83551EA3A72E_bingo,roleId=_bingo,strengthenKey=4C8818C1732D1A7B9733D611F28BD8B57923ADE220C9B60370F65B533E02E0F8E6A0FEFA632101D3D5409FE4F92580CF_bingo',
#夏香节
#5-2 9/12/18/22
'key=BCE52145EC2FBDDE212899674C8CA1C12A3A133EEA5D70CB9D998AF6B3F4648C22AC20BFEB2B797D1292322C150A0DC2_bingo,roleId=_bingo,strengthenKey=4C8818C1732D1A7B9733D611F28BD8B57923ADE220C9B60370F65B533E02E0F82F810FA48DCD7A31F76CAE44623F2470_bingo',
#15-8 9/12/15/18/20
'key=DDF1B71D0AF91A8547973CE5362A890F18C8E73AAC10BA9179CE5D2D745E95AC9AC125029761397270C947AC9F5E11CE_bingo,roleId=_bingo,strengthenKey=4C8818C1732D1A7B9733D611F28BD8B57923ADE220C9B60370F65B533E02E0F823A5E9624E2105020B0FB67EE3BFC671_bingo',
#19-5 0/10/18/20
'key=CB80DDB21929DB2DB9849A60F929CCB99B3BF927A75E71562ADB7CEC61846545338F6E4517C1973762A51B43E2C88518_bingo,roleId=_bingo,strengthenKey=4C8818C1732D1A7B9733D611F28BD8B57923ADE220C9B60370F65B533E02E0F8E6A0FEFA632101D3D5409FE4F92580CF_bingo',
#15-5 0/10/18/20
'key=751709DD793408174D56C44D3BFC3C434680AB42380C39D573683566316F1F090B3C468368A1FA0640C15E1A2A6BB25A_bingo,roleId=_bingo,strengthenKey=19F512DCD8EE34ABE9C4FB4A92C2F42A83880C27EA00E5A7DF564D095F19F994_bingo',
#10-3 16
'key=EDC82490F20A5D935C56A702456D489B3B9A0BF642892AE0C21FE1CC419C8F3BBED66A110B99B84B4750C2127C9846F4_bingo,roleId=_bingo,strengthenKey=E69C4C9B08504F0E61532E94C2391A4F3C8C17E33845A8E820806A9C43EC1E9AFE183C4CD0AF1C03DC7488C9405242B0_bingo'
    ]
args =Quanargs[4]
mycookies = [
]
# 10-2 7/10/16/20
# 19-5 0/10/18/22
# 5-2 9/12/18/22
# 15-8 9/12/15/18/20
# 19-5 0/10/18/20
# 15-5 0/10/18/20
# 10-3 16
QuanName=[
'10-2',
'19-5',
'5-2',
'15-8',
'19-5',
'15-5',
'10-3',
    ]
coupontime ="2022-05-16 09:53:00"
couponstamp=0
runFlag=[1,1,1]
starttime = 0  # 开始时间戳 13位 网址：https://tool.lu/timestamp/   5/8 5/7 23:59:58
range_n = 20  # 抢券次数
thread_n =1
range_sleep = 0.001#0.02/len(mycookies)  # 间隔时间
curr_time=0
# 没用的参数
log_list = []
atime = 0
deltatime=0
JDtime=0
dateArray=0
otherStyleTime=0
quanNum=0
quanData=[]
ResData=[]
ResTime=[]
bgtime=[]
ResRAW=[]
CountTable=[]
reqTruns=[]
fff =[]
runflag=1
changeTime=1000 #动态调参  根据系统修改 提前抢券时间

def get_log_list(num):
    global log_list
    try:
        for i in range(num):
            url = f'http://127.0.0.1:5889/log'
            res = requests.get(url=url).json()
            log_list.append(res)
    except:
        log_list = []
    return log_list
def randomString(e):
    t = "0123456789abcdef"
    a = len(t)
    n = ""
    for i in range(e):
        n = n + t[math.floor(random.random() * a)]
    return n
def Ua():
    UA = f'jdapp;iPhone;10.2.0;13.1.2;{randomString(40)};M/5.0;network/wifi;ADID/;model/iPhone8,1;addressid/2308460611;appBuild/167853;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 13_1_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1;'
    return UA
#i 抢券序号   j:券的序号 index:账号序号
def qiang_15_8(cookie,j,i, index):
    global ResData
    global ResTime
    global bgtime
    global CountTable
    global reqTruns
    global runFlag
    global quanData
    global fff
    global deltatime
    global couponstamp
    #bgtime.append(datetime.datetime.now())
    bgtime.append(datetime.datetime.fromtimestamp(time.time()+(deltatime)/1000))
    url = 'https://api.m.jd.com/client.action?functionId=lite_newBabelAwardCollection&client=wh5'
    headers = {
        "Accept": "*/*",
        "Accept-Encoding": "gzip, deflate, br",
        "Accept-Language": "zh-cn",
        "Connection": "keep-alive",
        "Content-Type": "application/x-www-form-urlencoded",
        'origin': 'https://pro.m.jd.com',
        "Referer": "https://pro.m.jd.com/jdlite/active/3H885vA4sQj6ctYzzPVix4iiYN2P/index.html?lng=106.476617&lat=29.502674&sid=fbc43764317f538b90e0f9ab43c8285w&un_area=4_50952_106_0",
        "Cookie": cookie,
        "User-Agent": Ua()
    }

    body = json.dumps({"activityId": "vN4YuYXS1mPse7yeVPRq4TNvCMR",
                       "scene": "1",
                       "args": Quanargs[j],
                       "log": log_list[i]['log'],
                       "random": log_list[i]['random']}
                      )
    data = f"body={body}"
    reqTruns.append(i)
    #print(cookie)
    CountTable.append(index + 1)
    '''
    if int(time.time()*1000)+deltatime>=couponstamp :
        print('已过抢券时间')
        sys.exit()
    '''
    try:
        #begin=datetime.datetime.now()
        res = requests.post(url=url, headers=headers, data=data).json()
        # print(res)
        if res['code'] == '0':
            #kk=string.find('d')
            #print(fff['mytime'])
            #print(res['subCodeMsg'].find('7此券今日已经被抢完'))
           # print(res['subCodeMsg'].find('89'))

            #ResData.append(res['subCodeMsg'])
            ResData.append(res)
            ResRAW.append(int(time.time() * 1000))
            ResTime.append(datetime.datetime.fromtimestamp(time.time()+deltatime/1000))
            ResRAW.append(int(time.time()*1000))
        else:
            print(f"账号{index + 1}：{res['errmsg']}")
    except:
        pass

def jdtime():
    outime=0.2
    url = 'http://api.m.jd.com/client.action?functionId=queryMaterialProducts&client=wh5'
    headers = {
        "user-agent": 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.159 Safari/537.36'
    }
    i = 0
    while i < 100:
        try:
            res = requests.get(url=url, headers=headers, timeout=outime).json()
            return int(res['currentTime2'])
        except:
            i += 1
            if outime <0.5:
                outime+=0.1
            print(f"超时了{i}")
    return 0
def get_CurQuan_ifo():

    global coupontime
    global couponstamp
    global quanNum
    global starttime
    global atime
    global runflag
    global changeTime
    global deltatime
    #获取下一个整点抢券时间
    #字符串

    #字符串转成时间格式
    couponstamp = int(time.mktime(time.strptime(coupontime, "%Y-%m-%d %H:%M:%S")) * 1000)
    coupontime=datetime.datetime.fromtimestamp(couponstamp/1000)
    # 10-2 7/10/16/20
    # 19-5 0/10/18/22
    # 5-2 9/12/18/22
    # 15-8 9/12/15/18/20
    # 19-5 0/10/18/20
    # 15-5 0/10/18/20
    # 10-3 16
    if coupontime.hour == 9 or  coupontime.hour == 12 or  coupontime.hour == 15 or  coupontime.hour == 18 or  coupontime.hour == 20  :
        quanNum=3
    elif coupontime.hour == 0 or coupontime.hour == 10:
        quanNum=5
    elif coupontime.hour == 22 :
        quanNum=2
    elif coupontime.hour == 16:
        quanNum=6
    else :
        quanNum=3
        #runflag=0
        print("当前时间没有抢券任务")
        #return

    print(f'优惠券额: {QuanName[quanNum]}')
    print(f"京东抢券时间:{coupontime}")
    starttime=couponstamp
    while True:
        if (starttime - int(time.time()*1000) )<= 200000:
            JDtime=jdtime()#当前京东时间
            curr_time = int(time.time()*1000)
            deltatime=JDtime-curr_time
            starttime = starttime - deltatime -changeTime
            print(f"京东时间:{datetime.datetime.fromtimestamp(JDtime / 1000)}")
            print(f"系统时间:{datetime.datetime.fromtimestamp(curr_time / 1000)}")
            if deltatime >0 :
                print(f"京东时间比系统时间快:{deltatime}ms")
            else :
                print(f"京东时间比系统时间慢:{deltatime}ms")
            print(f"系统抢券时间:{datetime.datetime.fromtimestamp(starttime/1000)}  ({deltatime}+{changeTime}))(比抢券时间早{changeTime}ms)")
            #print(f"还有{(starttime-curr_time)/1000}秒开始")
            atime=0
            break
        else:
            if int(time.time()*1000) - atime >= 30000:
                atime = int(time.time()*1000)
                print(f'等待获取log中，还差{int((starttime - int(time.time()*1000)) / 1000)}秒')


if __name__ == '__main__':
    print('极速版抢券准备...')
    coupontime=(datetime.datetime.now()+datetime.timedelta(hours=1)).strftime("%Y-%m-%d %H")+":00:00" #"2022-05-16 09:53:00"
    #coupontime=(datetime.datetime.now()).strftime("%Y-%m-%d %H")+":1:00" #"2022-05-16 09:53:00"
    get_CurQuan_ifo()
    #if runflag ==0:
        #sys.exit()

    get_log_list(range_n*len(mycookies)*thread_n)
    if len(log_list) != 0:
        print(f'{len(log_list)}条log获取完毕,再等待{int((starttime - int(time.time()*1000)) / 1000)}秒')
        tasks = list()
        for i in range(int(range_n)):
            for mm in range(len(mycookies)):
                tasks.append(threading.Thread(target=qiang_15_8, args=(mycookies[mm],quanNum, i*len(mycookies)+mm,mm)))
            #tasks.append(threading.Thread(target=qiang_15_8, args=(mycookies[0],quanNum, i*len(mycookies)+0,0)))
            #tasks.append(threading.Thread(target=qiang_15_8, args=(mycookies[1],quanNum, i*len(mycookies)+0,1)))
            #tasks.append(threading.Thread(target=qiang_15_8, args=(mycookies[2],quanNum, i*len(mycookies)+0,2)))
            #tasks.append(threading.Thread(target=qiang_15_8, args=(mycookies[2],quanNum, i*len(mycookies)+2,2)))
                #多线程就写多次
                #tasks.append(threading.Thread(target=qiang_15_8, args=(mycookies[mm], quanNum, i * len(mycookies) + mm, mm)))
        while True:
            curr_time = int(time.time() * 1000)
            if curr_time >= starttime:
                break
            else:
                if (starttime - curr_time) <= 10000:
                    if curr_time - atime >= 1000:
                        atime = curr_time
                        print(f'抢券倒计时{int((starttime - curr_time) / 1000)}秒')
                else:
                    atime=0




        for t in tasks:
            t.start()
            time.sleep(range_sleep)
        time.sleep(2)
        #for t in tasks:
            #t.join()
        #print(ResTime)
        for b in range(len(ResData)):
            for a in range(len(tasks)):
                if b==reqTruns[a]:
                    print(reqTruns[a],f'账号{CountTable[a]}',bgtime[b],ResTime[b],ResData[a])
              # print(reqTruns[b], f'账号{CountTable[b]}', bgtime[b], ResTime[b], ResData[b])

    else:
        print('暂无可用log')

